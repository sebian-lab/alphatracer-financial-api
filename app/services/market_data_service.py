...
def _adx(hi, lo, cl, p=14):
    ...
    pdi = (
        100 * pd.Series(pdm).ewm(span=p, adjust=False).mean() / atr_s.replace(0, np.nan)
    )
    mdi = (
        100 * pd.Series(mdm).ewm(span=p, adjust=False).mean() / atr_s.replace(0, np.nan)
    )
...
def _compute_signal(
    price, ma: MovingAverages, osc: Oscillators, vol: VolatilityIndicators
) -> TradingSignal:
    sig = {}
    for name, mv in [
        ("SMA20", ma.sma_20),
    ...
    sig["MACD"] = (
        ("BUY" if osc.macd_hist > 0 else "SELL")
        if osc.macd_hist is not None
        else "NEUTRAL"
    )
    sig["ADX"] = (
        ("BUY" if osc.di_plus > osc.di_minus else "SELL")
        if (osc.di_plus is not None and osc.di_minus is not None)
        else "NEUTRAL"
    )
    if vol.bb_lower and vol.bb_upper and price:
        sig["BB"] = (
            "BUY"
            if price < vol.bb_lower
            else ("SELL" if price > vol.bb_upper else "NEUTRAL")
        )
    else:
        sig["BB"] = "NEUTRAL"
...
def _build_analysis(ticker: str, interval: str, period: str) -> FullAnalysis:
    ...
    if df.empty:
        return FullAnalysis(
            quote=quote, interval=interval, period=period, bars_saved=bars_saved
        )
    ...
    vol_ind = VolatilityIndicators(
        ...
        bb_pct_b=_f((last_close - bl_v) / (bu_v - bl_v))
        if (bu_v and bl_v and last_close and bu_v != bl_v)
        else None,
    ...
    vol_ind2 = VolumeIndicators(
        obv=obv_val,
        volume=float(cur_vol) if cur_vol else None,
