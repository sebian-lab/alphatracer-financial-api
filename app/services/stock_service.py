...
class StockService:
    ...
    def load_tickers_from_csvs(self, db: Session = None) -> List[StockModel]:
        """
        ...
        try:
            ...
            for row in reader:
                try:
                    ticker = (
                        row.get(symbol_idx)
                        if callable(symbol_idx)
                        else symbol_idx(row)
                        if isinstance(symbol_idx, int)
                        else None
                    )
                    ...
                    data = {
                        ...
                        "name": row.get(name_idx).strip()
                        if callable(name_idx)
                        else name_idx(row).strip()
                        if isinstance(name_idx, int)
                        else "",
                        "sector": row.get(sector_idx).strip()
                        if callable(sector_idx)
                        else sector_idx(row).strip()
                        if isinstance(sector_idx, int)
                        else "Consumer Discretionary",
                        "industry": row.get(industry_idx).strip()
                        if callable(industry_idx)
                        else industry_idx(row).strip()
                        if isinstance(industry_idx, int)
                        else None,
                    }
    ...
    def search_stocks(
        self, query: str, limit: int = 10, db: Session = None
    ) -> List[Dict]:
        """
