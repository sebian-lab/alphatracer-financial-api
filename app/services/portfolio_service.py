...
from app.db.models import User as UserModel, Transaction as TxModel
...
def calculate_holdings(user: UserModel) -> dict:
    ...
    buys = TxModel.query.filter(TxModel.user_id == user.id, TxModel.type == "buy").all()

    sells = TxModel.query.filter(
        TxModel.user_id == user.id, TxModel.type == "sell"
    ).all()
    ...
    for ticker, buy_total in buy_totals.items():
        ...
        net_quantity = sum(
            b.quantity for b in buys if (b.stock and b.stock.ticker == ticker)
        )
